...
if ( ! class_exists( 'WpssoPlmRegister' ) ) {
	...
	class WpssoPlmRegister {
		...
		public function __construct() {
			...
			//register_deactivation_hook( WPSSOPLM_FILEPATH, array( $this, 'network_deactivate' ) );	// Deactivation method not required.
		...
		private function activate_plugin() {
			...
			$version = WpssoPlmConfig::$cf['plugin']['wpssoplm']['version'];	// only our config

			if ( class_exists( 'WpssoUtil' ) ) {
				WpssoUtil::save_all_times( 'wpssoplm', $version );
			} else {
				WpssoPlm::required_notice( true );			// $deactivate = true
			}
		}
