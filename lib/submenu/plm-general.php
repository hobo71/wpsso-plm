		...
		protected function get_table_rows( $metabox_id, $tab_key ) {
			...
			switch ( $metabox_id . '-' . $tab_key ) {
				...
				case 'plm-general':
					...
					$addr_names_none = WpssoPlmAddress::get_addr_names( '', true );	// $add_none is true.

					$table_rows['plm_addr_for_home'] = '' . 
					$this->form->get_th_html( _x( 'Address for a Blog Front Page', 'option label', 'wpsso-plm' ), '', 'plm_addr_for_home' ) . 
					'<td>' . $this->form->get_select( 'plm_addr_for_home',
						$addr_names_none, 'long_name', '', true, false, true ) . '</td>';
		
					$table_rows['plm_addr_def_country'] = '' . 
					$this->form->get_th_html( _x( 'Address Default Country', 'option label', 'wpsso-plm' ), '', 'plm_addr_def_country' ) . 
				...
				case 'contact-address':
					...
					$addr_names_new = WpssoPlmAddress::get_addr_names( '', false, true );	// $add_none = false, $add_new = true

					$first_num = SucomUtil::get_first_num( $addr_names_new );
					...
					$table_rows['plm_addr_id'] = '' . 
					$this->form->get_th_html( _x( 'Edit an Address', 'option label', 'wpsso-plm' ), '', 'plm_addr_id' ) . 
					'<td colspan="3">' . $this->form->get_select( 'plm_addr_id',
						$addr_names_new, 'long_name', '', true, false, true, 'unhide_rows' ) . '</td>';

					foreach ( $addr_names_new as $id => $name ) {

						$tr_html = '<!-- address id ' . $id . ' -->' . 
							'<tr class="hide_plm_addr_id hide_plm_addr_id_' . $id . '" style="display:none">';
		
						$table_rows['plm_addr_delete_' . $id] = $tr_html . $this->form->get_th_html() . 
					...
					foreach ( $addr_names_new as $id => $name ) {
						...
						$table_rows['plm_addr_name_' . $id] = $tr_html . 
						...
						$table_rows['plm_addr_streetaddr_' . $id] = $tr_html . 
						...
						$table_rows['plm_addr_po_box_number_' . $id] = $tr_html . 
						...
						$table_rows['plm_addr_zipcode_' . $id] = $tr_html . 
					...
					foreach ( $addr_names_new as $id => $name ) {
						...
						$table_rows['plm_addr_latitude_' . $id] = $tr_html . 
						...
						$table_rows['plm_addr_longitude_' . $id] = $tr_html . 
						...
						$table_rows['plm_addr_altitude_' . $id] = $tr_html . 
					...
					foreach ( $addr_names_new as $id => $name ) {
						...
						$table_rows['plm_addr_img_url_' . $id] = $tr_html . 
						...
						foreach ( $this->p->cf['form']['weekdays'] as $day => $day_label ) {
							...
							if ( $row_number === 1 ) {
								$th_cell = $tr_html . $this->form->get_th_html( _x( 'Business Days + Hours',
									'option label', 'wpsso-plm' ), '', 'plm_addr_days' );
							} else {
								$th_cell = $tr_html . '<th></th>';
							}
						...
						$table_rows['plm_addr_season_dates_' . $id] = $tr_html . 
						...
						$table_rows['plm_addr_service_radius_' . $id] = $tr_html . 
						...
						$table_rows['plm_addr_cuisine_' . $id] = $tr_html . 
						$this->form->get_th_html( _x( 'Serves Cuisine', 'option label', 'wpsso-plm' ), '', 'plm_addr_cuisine' ) .  
						'<td colspan="3">' . $this->form->get_input( 'plm_addr_cuisine_' . $id ) . '</td>';

						$table_rows['plm_addr_menu_url_' . $id] = $tr_html . 
						...
						$table_rows['plm_addr_order_urls_' . $id] = $tr_html . 
